    ...
    <div className="min-h-screen bg-[#F5F8FC]">
      ...
      <section className="pt-28 pb-10 px-6 md:px-12 max-w-6xl mx-auto text-center">
        ...
        >
          ...
          <p className="text-[#5B6B85] max-w-2xl mx-auto">
      ...
      <section className="px-6 md:px-12 max-w-6xl mx-auto grid md:grid-cols-5 gap-6 mb-16">
        ...
        >
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[#5B6B85] mb-1">Name</label>
              <input
                name="name"
                value={form.name}
          ...
          <div>
            <label className="block text-sm text-[#5B6B85] mb-1">Message</label>
            <textarea
              name="message"
              value={form.message}
              onChange={handleChange}
              required
              rows={5}
              className="w-full rounded-xl border border-white/60 bg-white/70 px-4 py-2.5 text-[#0F1729] outline-none focus:ring-2 focus:ring-[#2F6FED] resize-none"
            />
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-3 rounded-full bg-[#2F6FED] text-white font-medium hover:bg-[#2558c4] transition-colors disabled:opacity-60"
          >
          ...
          {status.message && (
            <p
              className={`text-sm ${
                status.state === "success" ? "text-[#14B8A6]" : "text-[#FF6B4A]"
              }`}
            >
              {status.message}
            </p>
        ...
        <div className="md:col-span-2 glass rounded-2xl p-6 md:p-8 space-y-5">
          <div>
            <p className="text-xs text-[#5B6B85] mb-1">Email</p>
            <p className="font-medium text-[#0F1729]">
              support@problemmapper.pk
            </p>
          </div>
          <div>
            <p className="text-xs text-[#5B6B85] mb-1">Response time</p>
            <p className="font-medium text-[#0F1729]">Within 24–48 hours</p>
          </div>
          <div>
            <p className="text-xs text-[#5B6B85] mb-1">Urgent civic issue?</p>
            <p className="text-sm text-[#5B6B85]">
              Use{" "}
              <a href="/report-issue" className="text-[#2F6FED] font-medium">
                Report an Issue
              </a>{" "}
          ...
          {OFFICES.map((office) => (
            <div key={office.city} className="glass rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="font-medium text-[#0F1729]">{office.city}</p>
              </div>
              <p className="text-sm text-[#5B6B85]">{office.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 md:px-12 max-w-6xl mx-auto mb-16 text-center">
        <p className="text-sm text-[#5B6B85] mb-3">Follow the project</p>
        <div className="flex justify-center gap-3">
          {["Twitter", "Facebook", "Instagram"].map((platform) => (
            <span
              key={platform}
              className="px-4 py-2 rounded-full glass text-sm text-[#5B6B85]"
            >
              {platform}
            </span>
          ))}
