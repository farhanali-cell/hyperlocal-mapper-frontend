  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#F5F8FC] flex items-center justify-center px-6">
        <p className="text-[#5B6B85]">Please log in to view your profile.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F5F8FC]">
      ...
      <section className="pt-28 pb-10 px-6 md:px-12 max-w-5xl mx-auto">
        ...
        >
          <div className="w-20 h-20 rounded-full bg-[#2F6FED] text-white flex items-center justify-center font-[Sora] text-2xl font-semibold shrink-0">
            {initialsOf(user.username)}
          </div>
          <div className="text-center sm:text-left">
            <h1 className="font-[Sora] text-2xl font-semibold text-[#0F1729]">
              {user.username}
            </h1>
            <p className="text-[#5B6B85] text-sm mb-2">{user.email}</p>
            <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-[#2F6FED]/10 text-[#2F6FED] capitalize">
              {user.role || "citizen"}
            </span>
          </div>
          <a
            href="/settings"
            className="sm:ml-auto px-5 py-2.5 rounded-full glass text-[#0F1729] font-medium hover:bg-white/70 transition-colors shrink-0"
          >
            Edit Profile
          </a>
        </motion.div>
      ...
      <section className="px-6 md:px-12 max-w-5xl mx-auto mb-12">
        ...
        <div className="glass rounded-2xl overflow-hidden">
          {loading ? (
            <p className="p-6 text-center text-[#5B6B85]">Loading…</p>
          ) : complaints.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-[#5B6B85] mb-4">
                You haven't reported anything yet.
              </p>
              <a
                href="/report-issue"
                className="inline-block px-5 py-2.5 rounded-full bg-[#2F6FED] text-white font-medium hover:bg-[#2558c4] transition-colors"
              >
                Report an Issue
              </a>
            </div>
            ...
            <div className="divide-y divide-white/40">
              {complaints.map((c) => (
                ...
                >
                  ...
                  <span className="text-xs text-[#5B6B85] capitalize shrink-0">
      ...
      {/* 4. Impact note */}
      <section className="px-6 md:px-12 max-w-5xl mx-auto mb-12">
        <div className="rounded-2xl bg-[#0F1729] text-white p-6 md:p-8 flex items-center gap-4">
          <span className="w-3 h-3 rounded-full bg-[#14B8A6] animate-pulse shrink-0" />
